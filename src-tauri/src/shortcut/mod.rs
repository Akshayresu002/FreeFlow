...
// Note: Commands are accessed via shortcut::freeflow_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::FreeFlowKeys => {
            if let Err(e) = freeflow_keys::init_shortcuts(app) {
                ...
                // Update settings to persist the fallback so we don't retry FreeFlowKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::FreeFlowKeys => freeflow_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::FreeFlowKeys => freeflow_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::FreeFlowKeys => freeflow_keys::register_shortcut(app, binding),
    }
}
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (FreeFlowKeys needs state)
    if new_impl == KeyboardImplementation::FreeFlowKeys {
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::FreeFlowKeys => "handy_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::FreeFlowKeys => freeflow_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "handy_keys" => KeyboardImplementation::FreeFlowKeys,
        other => {
            warn!(
...
fn initialize_handy_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    ...
    if let Err(e) = freeflow_keys::init_shortcuts(app) {
        error!("Failed to initialize FreeFlowKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize FreeFlowKeys: {}. Reverted to Tauri.",
            e
        ));
    }
