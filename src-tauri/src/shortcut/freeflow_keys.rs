//! FreeFlow-keys based keyboard shortcut implementation
//!
...
impl FreeFlowKeysState {
    /// Create a new FreeFlowKeysState
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            let event = {
                let state = match app.try_state::<FreeFlowKeysState>() {
                    Some(s) => s,
                    None => break,
                };
...
impl Drop for FreeFlowKeysState {
    fn drop(&mut self) {
...
/// Validate a shortcut string for the FreeFlowKeys implementation.
/// FreeFlowKeys is more permissive: allows modifier-only combos and the fn key.
pub fn validate_shortcut(raw: &str) -> Result<(), String> {
    if raw.trim().is_empty() {
        return Err("Shortcut cannot be empty".into());
    }
    // FreeFlowKeys accepts modifier-only, key-only, and modifier+key combos
    // Just verify the string is parseable
    raw.parse::<Hotkey>()
        .map(|_| ())
        .map_err(|e| format!("Invalid shortcut for FreeFlowKeys: {}", e))
}

/// Initialize handy-keys shortcuts
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    let state = FreeFlowKeysState::new(app.clone())?;
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Some(cancel_binding) = get_settings(&app_clone).bindings.get("cancel").cloned() {
                if let Some(state) = app_clone.try_state::<FreeFlowKeysState>() {
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    let state = app
        .try_state::<FreeFlowKeysState>()
        .ok_or("FreeFlowKeysState not initialized")?;
...
pub fn start_handy_keys_recording(app: AppHandle, binding_id: String) -> Result<(), String> {
    ...
    if settings.keyboard_implementation != settings::KeyboardImplementation::FreeFlowKeys {
