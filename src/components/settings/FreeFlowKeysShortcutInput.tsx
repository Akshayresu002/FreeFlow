...
interface FreeFlowKeysShortcutInputProps {
...
export const FreeFlowKeysShortcutInput: React.FC<FreeFlowKeysShortcutInputProps> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopFreeFlowKeysRecording().catch(console.error);
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<FreeFlowKeysEvent>(
        "handy-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopFreeFlowKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await commands.startFreeFlowKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {
