...
This guide covers how to set up the development environment and build FreeFlow from source across different platforms.
...
```bash
git clone git@github.com:freeflow/app.git
cd FreeFlow
```
...
```bash
cd /tmp
ar x /path/to/FreeFlow/src-tauri/target/release/bundle/deb/FreeFlow_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/handy /usr/bin/
sudo cp -r usr/lib/FreeFlow /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/FreeFlow.desktop /usr/share/applications/
```
...
```
Bundling FreeFlow_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir FreeFlow.AppDir --plugin gtk --output appimage
```
